...
# -*- coding: utf-8 -*-
"""
VRP Optimizer Pro v4 - Serveur Flask avec Ollama
Normalisation adresses â†’ GÃ©ocodage intelligent â†’ Routing IA â†’ Dashboard
"""
...
# âš ï¸ IMPORTANT: Choisir le modÃ¨le adaptÃ© Ã  vos ressources
# - gemma2:2b (RAPIDE - recommandÃ© pour < 8GB RAM)
# - gemma3:4b (plus lent mais meilleur qualitÃ©)
# Pour changer: ollama pull gemma2:2b ou gemma3:4b
OLLAMA_MODEL  = "gemma3:4b"  # Retour Ã  gemma3:4b pour meilleure qualitÃ©
...
ADDRESS_NORMALIZATION = {
    ...
    'all': 'allÃ©e', 'all.': 'allÃ©e',
    'crs': 'cours', 'cours': 'cours',
    'ch': 'chemin', 'ch.': 'chemin',
    'ctl': 'castÃ©rale', 'ctle': 'castÃ©rale',
    'quai': 'quai', 'q': 'quai',
    ...
    'citÃ©': 'citÃ©', 'cite': 'citÃ©', 'cte': 'citÃ©',
    'rÃ©s': 'rÃ©sidence', 'res': 'rÃ©sidence', 'rÃ©sidence': 'rÃ©sidence',
    'lot': 'lotissement', 'lot.': 'lotissement',
    'zac': 'zone d\'activitÃ© commerciale', 'z.i': 'zone industrielle',
    'za': 'zone d\'activitÃ©', 'zad': 'zone d\'amÃ©nagement diffÃ©rÃ©',
    
    # PrÃ©fixes numÃ©riques
    'bis': 'bis', 'ter': 'ter', 'quater': 'quater',
    'b': 'bis', 't': 'ter', 'q': 'quater',
    
    # Autres abrÃ©viations courantes
    'st': 'saint', 'ste': 'sainte',
    'mle': 'maison', 'maison': 'maison',
    'bat': 'bÃ¢timent', 'bÃ¢t': 'bÃ¢timent', 'bÃ¢timent': 'bÃ¢timent',
    'etg': 'Ã©tage', 'Ã©tage': 'Ã©tage',
    'apt': 'appartement', 'app': 'appartement',
    'bp': 'boÃ®te postale', 'b.p': 'boÃ®te postale',
...
# Codes postaux franÃ§ais pour validation
...
def _make_session():
    ...
    s.mount("http://", HTTPAdapter(max_retries=retry))  # âœ… Ajouter HTTP aussi !
    return s
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# PROGRESSION TEMPS-RÃ‰EL (SSE)
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
progress_store: dict = {}
progress_lock  = threading.Lock()

def set_progress(pid: str, pct: int, msg: str = "", done: bool = False):
    """Met Ã  jour la progression pour un progress_id donnÃ©."""
    if not pid:
        return
    with progress_lock:
...
def api_progress_stream(progress_id):
    """SSE : streame la progression d'une opÃ©ration longue."""
    def generate():
        last_pct = -1
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# AUTO-DÃ‰TECTION ET MAPPAGE DES COLONNES
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
FIELD_META = {
    ...
    'date_min':   {'label': 'Date dÃ©but',            'required': False, 'keywords': ['date_min','debut','from','start','d_min','date_debut','date_from','ouverture','datemin']},
...
def _norm_col(s: str) -> str:
    ...
    s = s.lower().strip()
    for acc, plain in [('Ã©','e'),('Ã¨','e'),('Ãª','e'),('Ã«','e'),('Ã ','a'),('Ã¢','a'),
                       ('Ã¹','u'),('Ã»','u'),('Ã®','i'),('Ã¯','i'),('Ã´','o'),('Ã§','c')]:
        s = s.replace(acc, plain)
    return re.sub(r'[^a-z0-9]', '_', s)

def auto_detect_columns(columns: list) -> dict:
    """Retourne {field: colonne_source | None} par correspondance de mots-clÃ©s."""
    ...
    for field, info in FIELD_META.items():
        ...
        for col, nc in norm.items():
            if nc == field:
                match = col; break
        if not match:
            # 2. Un mot-clÃ© contenu dans le nom de colonne (ou l'inverse)
            for col, nc in norm.items():
...
def api_preview_columns():
    """Analyse un fichier Excel : retourne colonnes, suggestions de mappage et aperÃ§u."""
    try:
...
def normalize_address(address, ville):
    ...
    Retourne l'adresse normalisÃ©e et les corrections appliquÃ©es
    """
    ...
    # Ã‰tape 1 : Nettoyage initial (caractÃ¨res spÃ©ciaux, espaces multiples)
    original = re.sub(r'[;,/_]', ' ', original)
    original = re.sub(r'\s+', ' ', original).strip()
    
    # Ã‰tape 2 : Remplacements intelligents par ordre de prioritÃ©
    # On utilise des regex avec word boundaries pour Ã©viter les remplacements partiels
    
    replacements = [
        # Types de voies (avec word boundaries pour Ã©viter les remplacements dans les mots)
        (r'\bBD\b', 'boulevard'),
        (r'\bB\.D\.', 'boulevard'),
        (r'\bBLD\b', 'boulevard'),
        (r'\bAV\b', 'avenue'),
        ...
        (r'\bZAC\b', 'zone d\'activitÃ© commerciale'),
        (r'\bZ\.I\.', 'zone industrielle'),
        (r'\bZA\b', 'zone d\'activitÃ©'),
        ...
        (r'\bBP\b', 'boÃ®te postale'),
        (r'\bB\.P\.', 'boÃ®te postale'),
        (r'\bCEDEX\b', 'cedex'),
        (r'\bCX\b', 'cedex'),
        (r'\bCS\b', 'coursier'),
        (r'\bC\.S\.', 'coursier'),
    ...
    # Ã‰tape 3 : Appliquer les remplacements avec word boundaries
    normalized = original.lower()
    for pattern, replacement in replacements:
        old = normalized
        normalized = re.sub(pattern, replacement, normalized, flags=re.IGNORECASE)
        if old != normalized:
            corrections.append(f"{pattern} â†’ {replacement}")
    
    # Ã‰tape 4 : Nettoyage final
    normalized = re.sub(r'\s+', ' ', normalized).strip()
    normalized = normalized.title()
    
    # Ã‰tape 5 : Ajout de la ville si pas prÃ©sente
...
def validate_address_format(address):
    ...
    # VÃ©rifier prÃ©sence d'un numÃ©ro de rue
    if not re.search(r'\b\d+\b', address):
        errors.append("Pas de numÃ©ro de rue dÃ©tectÃ©")
        suggestions.append("Ajoutez un numÃ©ro si possible")
    
    # VÃ©rifier prÃ©sence d'un type de voie
    voie_types = ['rue', 'avenue', 'boulevard', 'place', 'chemin', 'impasse', 
                  'allÃ©e', 'cours', 'route', 'square', 'quai', 'rÃ©sidence']
    has_voie = any(vt in address.lower() for vt in voie_types)
    if not has_voie:
        errors.append("Type de voie non dÃ©tectÃ©")
        suggestions.append("VÃ©rifiez le type de voie (rue, avenue, etc.)")
    
    # VÃ©rifier code postal
    has_cp = POSTAL_CODES_PATTERN.search(address) is not None
    if not has_cp:
        errors.append("Code postal non dÃ©tectÃ©")
        suggestions.append("Ajoutez le code postal pour amÃ©liorer la prÃ©cision")
    
    return len(errors) == 0, errors, suggestions

# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# GÃ‰OCODAGE ROBUSTE - TOUJOURS TROUVER UN RÃ‰SULTAT
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

# Centres approximatifs des dÃ©partements franÃ§ais (code postal -> lat, lon)
DEPT_CENTERS = {
...
# Cache gÃ©ocodage en mÃ©moire (clÃ© = adresse normalisÃ©e, valeur = rÃ©sultat)
_geocode_cache: dict = {}

def _try_geocode(address, timeout=5):  # â¬‡ï¸ RÃ©duit de 10s Ã  5s
    """
    Tente de gÃ©ocoder une adresse via Nominatim
    Retourne None si Ã©chouÃ©
    """
    try:
...
def _geocode_one(address, raw_address=None):
    """
    GÃ©ocode une adresse avec STRATÃ‰GIES MULTIPLES
    NE JAMAIS RETOURNER None - TOUJOURS TROUVER UN RÃ‰SULTAT
    Utilise un cache en mÃ©moire pour Ã©viter les requÃªtes redondantes.
    """
    if not address:
        log.warning("âš ï¸  Adresse vide - Utilisation centre France")
        return {
            "lat": 46.603354,
            "lon": 1.888334,
    ...
    # Extraire la ville (aprÃ¨s le code postal)
    if code_postal:
    ...
    strategies = [
        raw_address,  # StratÃ©gie 0: Adresse brute originale (avant normalisation)
        address,  # StratÃ©gie 1: Adresse normalisÃ©e
        f"{code_postal} {ville}".strip() if code_postal and ville else None,  # StratÃ©gie 2: CP + ville
        ville if ville else None,  # StratÃ©gie 3: Juste ville
    ]
    
    # Filtrer les stratÃ©gies None/vides
    strategies = [s for s in strategies if s and len(s) > 2]
    
    # Essayer chaque stratÃ©gie
    for i, strategy in enumerate(strategies):
        log.debug(f"ðŸ”Ž Tentative {i+1}/{len(strategies)}: {strategy[:40]}")
        result = _try_geocode(strategy, timeout=3)  # RÃ©duit Ã  3s
        if result:
            log.info(f"âœ… TrouvÃ© (strat {i}): {strategy[:40]} â†’ {result['lat']:.4f}, {result['lon']:.4f}")
            _geocode_cache[address] = result
            return result

        # DÃ©lai Nominatim entre stratÃ©gies
        if i < len(strategies) - 1:
            time.sleep(GEOCODE_DELAY)
    if dept_code and dept_code in DEPT_CENTERS:
        lat, lon = DEPT_CENTERS[dept_code]
        log.warning(f"âš ï¸  Fallback dÃ©partement {dept_code}: {address[:40]} â†’ {lat:.4f}, {lon:.4f}")
        result = {
            "lat": lat,
            "lon": lon,
            "display_name": f"Centre dÃ©partement {dept_code} ({code_postal})",
            "is_fallback": True
        }
        _geocode_cache[address] = result
        return result
    ...
    log.error(f"ðŸš¨ ULTIME RECOURS: {address[:40]} â†’ Centre France")
    return {
        "lat": 46.603354,
        "lon": 1.888334,
...
def geocode_with_validation(raw_rows, session_id, progress_callback=None):
    """
    GÃ©ocode les sites avec validation et retourne les rÃ©sultats.
    progress_callback(pct, msg) est appelÃ© aprÃ¨s chaque adresse.
    """
    results = {
        "success": [],
        "failed": [],
    ...
    log.info("ðŸ” GÃ©ocodage de %d adresses avec validation", len(raw_rows))

    for i, row in enumerate(raw_rows):
        log.debug(f"[{i+1}/{len(raw_rows)}] DÃ©but traitement adresse {i}")
        row_dict = dict(row)
        ...
        # VÃ©rifier coordonnÃ©es dÃ©jÃ  prÃ©sentes
        try:
            ...
            if math.isfinite(lat) and math.isfinite(lon):
                ...
                results["geocoded"] += 1
                log.info("[%d/%d] âœ… CoordonnÃ©es directes", i+1, len(raw_rows))
                continue
        ...
        # Tenter le gÃ©ocodage (le sleep Nominatim est sautÃ© si cache hit)
        log.debug(f"[{i+1}] Geocodage de: {normalized[:50]}")
        cache_hit = normalized in _geocode_cache
        # Construire la requÃªte brute (sans normalisation) pour la stratÃ©gie 0
        ...
        log.debug(f"[{i+1}] RÃ©sultat: lat={geo_result.get('lat')}, lon={geo_result.get('lon')}, fallback={geo_result.get('is_fallback', False)}")
        ...
        if geo_result:
            ...
            if geo_result.get("is_fallback"):
                results["warnings"].append({
                    ...
                    "message": f"CoordonnÃ©es approximatives: {geo_result['display_name']}",
                    "original": original_address,
                    "normalized": normalized
                })
            ...
            results["success"].append(row_dict)
            results["geocoded"] += 1
            log.info("[%d/%d] %s %s â†’ %.5f, %.5f%s", i+1, len(raw_rows),
                    "âš¡" if cache_hit else "âœ…",
                    normalized[:30], geo_result["lat"], geo_result["lon"],
                    " (fallback)" if geo_result.get("is_fallback") else "")
        else:
            log.error("[%d/%d] âŒ ERREUR CRITIQUE: %s", i+1, len(raw_rows), normalized)
        ...
        # DÃ©lai Nominatim uniquement si une vraie requÃªte HTTP a Ã©tÃ© faite
        if not cache_hit:
        ...
        if progress_callback:
            ...
            progress_callback(pct, f"GÃ©ocodage {i+1}/{len(raw_rows)} : {normalized[:35]}")

    # Sauvegarder les rÃ©sultats pour validation manuelle
...
def _site_name_candidates(row):
    """Retourne les identifiants texte possibles d'un site (normalisÃ©s)."""
    keys = (
...
def _norm_site_token(value):
    """Normalise un identifiant site/dÃ©pÃ´t pour comparaison robuste."""
...
def _row_contains_depot_keyword(row):
    """Heuristique de secours: une ligne ressemble au dÃ©pÃ´t."""
...
def _find_depot_index(rows, depot_name):
    """Trouve l'index du dÃ©pÃ´t dans une liste de lignes."""
    ...
    if len(fallback) == 1:
        log.warning("  âš ï¸ DÃ©pÃ´t retrouvÃ© via heuristique 'depot' (format de nom diffÃ©rent)")
        return fallback[0]
    return None
...
def ask_ollama(prompt, model=OLLAMA_MODEL, timeout=300):
    """
    Pose une question Ã  Ollama. Timeout 300s, num_ctx rÃ©duit pour accÃ©lÃ©rer.
    """
    try:
        log.info("ðŸ“ Envoi prompt Ollama (timeout=%ds, model=%s, len=%d)", timeout, model, len(prompt))
        if not model:
            return None
        ...
        log.info("âœ… RÃ©ponse Ollama reÃ§ue (%d caractÃ¨res)", len(response_text))
        return response_text
    except requests.exceptions.Timeout:
        log.error("â±ï¸ TIMEOUT Ollama aprÃ¨s %ds", timeout)
        return None
    except requests.exceptions.ConnectionError:
        log.error("âŒ Ollama non accessible sur %s", OLLAMA_URL)
        return None
    except Exception as e:
        log.error("âŒ Erreur Ollama: %s", str(e))
        return None


def _build_report_from_data(routes_data, total_tours, total_stops, total_km, total_h, avg_stops, avg_km, avg_h, eff_ratio):
    """
    GÃ©nÃ¨re un rapport analytique complet en Python pur, sans IA.
    UtilisÃ© comme fallback quand Ollama timeout ou est indisponible.
    """
    lines_out = []

    # 1. SynthÃ¨se
    lines_out.append("## ðŸ“Š SYNTHÃˆSE GLOBALE")
    if avg_h < 4:
        charge_eval = "lÃ©gÃ¨res (< 4h/tournÃ©e)"
    elif avg_h < 7:
        charge_eval = "bien dimensionnÃ©es (4-7h/tournÃ©e)"
    else:
        charge_eval = "chargÃ©es (> 7h/tournÃ©e)"
    lines_out.append(
        f"Le plan comprend {total_tours} tournÃ©es pour {total_stops} visites, "
        f"{total_km:.0f} km au total. Les tournÃ©es sont {charge_eval}. "
        f"DurÃ©e moyenne : {avg_h:.1f}h | Distance moyenne : {avg_km:.0f} km. "
        f"Vitesse commerciale estimÃ©e : {eff_ratio:.0f} km/h."
    )
    lines_out.append("")

    # 2. Ã‰quilibrage
    lines_out.append("## âš–ï¸ Ã‰QUILIBRAGE DES CHARGES")
    ...
    if overloaded:
        for r in overloaded:
            lines_out.append(f"âš ï¸  T{r['num']} ({r['city']}) : {r['stops']} arrÃªts â€” SURCHARGÃ‰E (moy. {avg_stops:.0f}). Envisager de diviser.")
    if underloaded:
        for r in underloaded:
            lines_out.append(f"â„¹ï¸  T{r['num']} ({r['city']}) : {r['stops']} arrÃªts â€” SOUS-CHARGÃ‰E (moy. {avg_stops:.0f}). Envisager de fusionner avec une tournÃ©e proche.")
    if not overloaded and not underloaded:
        lines_out.append(f"âœ… Ã‰quilibrage satisfaisant â€” toutes les tournÃ©es sont proches de la moyenne ({avg_stops:.0f} arrÃªts).")
    lines_out.append("")

    # 3. NuitÃ©es
    lines_out.append("## ðŸ¨ LOGISTIQUE & NUITÃ‰ES")
    long_tours = [r for r in routes_data if r['time_hours'] > 8]
    if long_tours:
        for r in long_tours:
            lines_out.append(f"ðŸŒ™ T{r['num']} ({r['city']}) : {r['time_hours']:.1f}h â€” NuitÃ©e probable. PrÃ©voir hÃ©bergement Ã  {r['city']}.")
    else:
        max_h = max(r['time_hours'] for r in routes_data)
        lines_out.append(f"âœ… Aucune nuitÃ©e nÃ©cessaire â€” durÃ©e maximum : {max_h:.1f}h.")
    lines_out.append("")

    # 4. Recommandations
    lines_out.append("## ðŸ’¡ RECOMMANDATIONS")
    reco_idx = 1

    if overloaded:
        names = ", ".join(f"T{r['num']}" for r in overloaded)
        lines_out.append(f"R{reco_idx}. Diviser les tournÃ©es surchargÃ©es : {names}")
        reco_idx += 1
    if underloaded:
        names = ", ".join(f"T{r['num']}" for r in underloaded)
        lines_out.append(f"R{reco_idx}. Fusionner les tournÃ©es sous-chargÃ©es : {names}")
        reco_idx += 1

    # TournÃ©es avec fort temps de visite
    ...
    if heavy_visit:
        ...
        lines_out.append(f"R{reco_idx}. Sites Ã  temps de visite Ã©levÃ© ({names}) : les placer en dÃ©but de tournÃ©e pour optimiser la fatigue.")
        reco_idx += 1

    # TournÃ©es longues en distance
    long_km = [r for r in routes_data if r['distance_km'] > avg_km * 1.5]
    if long_km:
        names = ", ".join(f"T{r['num']} ({r['distance_km']:.0f}km)" for r in long_km)
        lines_out.append(f"R{reco_idx}. RÃ©duire la distance des tournÃ©es Ã©loignÃ©es : {names} â€” vÃ©rifier si des sites peuvent Ãªtre rÃ©affectÃ©s Ã  des tournÃ©es voisines.")
        reco_idx += 1

    if reco_idx == 1:
        lines_out.append("âœ… Le plan est globalement bien optimisÃ©. Aucune recommandation critique.")
    lines_out.append("")

    # 5. DÃ©tail
    lines_out.append("## ðŸš¨ TOURNÃ‰ES CRITIQUES")
    critical = [r for r in routes_data if r['stops'] < 2 or r['time_hours'] > 8 or r['distance_km'] > 400 or r['stops'] > avg_stops * 1.8]
    if critical:
        for r in critical:
            issues = []
            if r['stops'] < 2: issues.append("trop peu d'arrÃªts â†’ fusionner")
            if r['time_hours'] > 8: issues.append(f"durÃ©e {r['time_hours']:.1f}h â†’ prÃ©voir nuitÃ©e ou diviser")
            if r['distance_km'] > 400: issues.append(f"distance {r['distance_km']:.0f}km â†’ rÃ©affecter sites lointains")
            if r['stops'] > avg_stops * 1.8: issues.append(f"{r['stops']} arrÃªts â†’ diviser en 2 tournÃ©es")
            lines_out.append(f"âš¡ T{r['num']} ({r['city']}) : {' | '.join(issues)}")
    else:
        lines_out.append("âœ… Aucune tournÃ©e critique dÃ©tectÃ©e.")
...
def analyze_routes_with_ai(routes, sites, time_m, dist_m, depot_idx):
    """
    Analyse les tournÃ©es.
    StratÃ©gie : 1 seul appel Ollama ultra-court, timeout 300s.
    Fallback Python pur si Ollama timeout ou indisponible.
    """
    max_routes_to_analyze = min(len(routes), 20)
    routes_to_analyze = routes[:max_routes_to_analyze]
    log.info("ðŸ§  Analyse : %d tournÃ©es", len(routes_to_analyze))

    # â”€â”€ Calcul des mÃ©triques â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    routes_data = []
    ...
    # â”€â”€ RÃ©sumÃ© compact pour le prompt â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # On encode chaque tournÃ©e en une ligne trÃ¨s courte pour tenir dans num_ctx=512
    short_summary = " | ".join(
    ...
    prompt = (
        f"Expert logistique. {total_tours} tournÃ©es, {total_stops} visites, "
        f"{total_km:.0f}km, {total_h:.0f}h. Moy:{avg_stops:.0f}arr/{avg_km:.0f}km/{avg_h:.1f}h. "
        f"DonnÃ©es: {short_summary}. "
    ...
    log.info("ðŸ“¤ Un seul appel Ollama (prompt %d chars)", len(prompt))
    ai_text = ask_ollama(prompt, timeout=300)

    # â”€â”€ Fallback Python si Ollama ne rÃ©pond pas â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if not ai_text:
        log.warning("âš ï¸  Ollama indisponible â€” gÃ©nÃ©ration du rapport en mode Python pur")
    ...
    # â”€â”€ Rapport avec rÃ©ponse Ollama â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Ã‰galement enrichi des donnÃ©es calculÃ©es en Python pour les mÃ©triques
    ...
    final_report = (
        "# ðŸ” RAPPORT D'ANALYSE LOGISTIQUE\n\n"
        "## ðŸ¤– ANALYSE IA\n" + ai_text +
        "\n\n---\n\n" + python_extras
    )

    log.info("âœ… Rapport gÃ©nÃ©rÃ© (IA + donnÃ©es calculÃ©es)")
    return {
        "success": True,
...
def run_vrp_robust(time_m, depot_idx, max_hours, solver_time_s, sites, max_s=14400):
    """
    Optimisation VRP basÃ©e sur le TEMPS TOTAL (trajet + temps sur site)
    PlutÃ´t que sur le nombre de sites.
    
    Args:
        time_m: Matrice des temps de trajet
        depot_idx: Index du dÃ©pÃ´t
        max_hours: DurÃ©e maximale par tournÃ©e en heures
    ...
    log.info(f"  â±ï¸  Contrainte temps par tournÃ©e: {max_hours}h ({max_time_seconds}s)")
    
    # Extraire les temps de visite des sites (en minutes)
    visit_times = []
    for i, site in enumerate(sites):
        if i == depot_idx:
            visit_times.append(0)  # Pas de temps de visite au dÃ©pÃ´t
        else:
            visit_times.append(_visit_minutes_from_row(site, default=30) * 60)
    
    log.info(f"  â±ï¸  Temps de visite moyens: {sum(visit_times)/len(visit_times)/60:.1f} min/site")
    
    # CrÃ©er une matrice de temps incluant les temps de visite
    # Pour chaque arc (i->j), le temps total = trajet(i->j) + temps_visite(i)
    # Sauf pour le dÃ©pÃ´t qui a un temps de visite = 0
    ...
    # Estimer le nombre de vÃ©hicules nÃ©cessaires
    # Somme des temps de visite / temps max par vÃ©hicule
    total_visit_time = sum(visit_times)
    estimated_vehicles = max(1, min(n-1, int(math.ceil(total_visit_time / max_time_seconds * 1.2))))
    log.info(f"  ðŸšš  Estimation vÃ©hicules: {estimated_vehicles}")
    ...
    routing.AddDimension(
        ...
        max_time_seconds,  # temps maximum par vÃ©hicule
        True,  # commencer Ã  0 (au dÃ©pÃ´t)
        'Time'
    )
    
    # Fixer le coÃ»t Ã  minimiser sur la dimension Temps
    ...
    # Utiliser un guided local search pour de meilleurs rÃ©sultats
    ...
    if solution:
        ...
        log.info(f"  âœ… Solution trouvÃ©e: {len(routes)} tournÃ©es")
        return routes
    else:
        log.warning("âš ï¸  Pas de solution avec contrainte de temps, tentative sans contrainte")
...
def _run_vrp_no_constraint(time_m, depot_idx, max_hours, sites):
    ...
    UtilisÃ© quand la solution avec contrainte Ã©choue
    """
    n = len(time_m)
    if n <= 1:
        return []
...
def run_vrp_with_date_groups(time_m, dist_m, sites, depot_idx, date_groups, group_size, solver_time_s):
    """
    Optimisation VRP avec contraintes de fenÃªtres temporelles
    Chaque groupe de dates est optimisÃ© sÃ©parÃ©ment
    """
    all_routes = []
    ...
    for group_idx, group in enumerate(date_groups):
        ...
        for item in group["sites"]:
            # GÃ©rer le mix entre objets wrappÃ©s et lignes brutes
        ...
        log.info(f"ðŸ“… Optimisation groupe {group_idx + 1}/{len(date_groups)} : {group_n} sites (dont dÃ©pÃ´t)")
        
        # CrÃ©er la liste de sites pour ce groupe (dans l'ordre group_site_indices)
        group_sites_list = [sites[i] for i in group_site_indices]
        
        # Optimiser ce groupe
        # Dans la matrice rÃ©duite, le dÃ©pÃ´t est toujours Ã  la position group_site_indices.index(depot_idx)
    ...
    log.info(f"ðŸ“… {len(all_routes)} tournÃ©es totales gÃ©nÃ©rÃ©es")
    return all_routes
...
def split_long_tours_by_time(routes, time_m, depot_idx, max_s, split_size, sites, split_mode="manual", dist_m=None, use_ai=False):
    ...
    # En mode auto, pas de plafond artificiel : les tournées se remplissent
    ...
    def _route_metric(route):
        """Coût de minimisation: km si dist_m disponible, sinon temps de trajet."""
        if not route:
            return 0.0
        seq = [depot_idx] + route + [depot_idx]
    ...
    def _merge_small_chunks(chunks, min_sz, max_sz, strict=True):
        ...
        while _has_small_chunk(chunks, min_sz) and len(chunks) > 1 and guard < 500:
            ...
            for j, donor in enumerate(chunks):
                ...
                if donor:
                    ...
                    donor_options.append((donor[1:], donor[0]))    # prendre le début
            ...
            # 2) Sinon fusionner le petit morceau avec la meilleure tournée voisine
        ...
        if _has_small_chunk(chunks, min_sz):
            log.warning(f"     ⚠️  Morceau < {min_sz} sites conservé (mode {'strict' if strict else 'relax'})")
        return chunks
    ...
    def _manual_split_route(route):
        ...
        # Éviter un dernier morceau à 1 site
    ...
    def _dp_partition_auto(route):
        """Partition dynamique minimisant le coût total (km/temps) sous contraintes."""
        n = len(route)
        inf = float("inf")
        dp = [inf] * (n + 1)
        nxt = [-1] * (n + 1)
        ...
        for i in range(n - 1, -1, -1):
            ...
            for j in range(i + min_sites, max_j + 1):
                ...
                # Minimiser le coût de trajet pur (km ou temps) + pénalité par tournée
                # pour favoriser des tournées plus remplies (moins de tournées au total)
    ...
    def _choose_merge_with_ai(candidates):
        ...
        if not use_ai or not OLLAMA_MODEL or ai_merge_budget <= 0:
            return candidates[0]
        ...
        try:
            ai_merge_budget -= 1
            ans = ask_ollama(prompt, timeout=25)
            if ans:
    ...
    if split_mode == "manual":
        ...
        for r in routes:
            ...
            if len(r) > fixed_size:
                log.info(f"  ✂️  Découpage manuel fixe: {len(r)} sites, {total_time/3600:.1f}h (taille {fixed_size})")
            chunks = _manual_split_route(r)
            for ch in chunks:
                ch_time = _route_total_seconds(ch)
                if ch_time > max_s and len(ch) > 1:
                    log.warning(f"     ⚠️  Morceau fixe long: {len(ch)} sites, {ch_time/3600:.1f}h > {max_s/3600:.1f}h")
    ...
    # Mode auto: découpage variable — chaque tournée est remplie au maximum du temps disponible
    final = []
    for r in routes:
        if not r:
            continue
        ...
        log.info(
            f"  ✂️  Découpage auto intelligent: {len(r)} sites, {total_time/3600:.1f}h "
            f"(max {max_s/3600:.1f}h/tournée, min {min_sites} sites)"
        )
        ...
        for ch in chunks:
            ch_time = _route_total_seconds(ch)
            if ch_time > max_s and len(ch) > 1:
                log.warning(f"     ⚠️  Morceau auto long: {len(ch)} sites, {ch_time/3600:.1f}h > {max_s/3600:.1f}h")
            log.info(f"     → Nouveau morceau: {len(ch)} sites, {ch_time/3600:.1f}h")
        final.extend(chunks)

    # Dernière passe globale pour éliminer les morceaux à 1 site quand possible
...
def build_excel(tours_data):
    """GÃ©nÃ¨re le fichier Excel des tournÃ©es"""
...
def get_date_windows(sites):
    ...
    if not dated_sites:
        log.info("ðŸ“… Aucune date trouvÃ©e, crÃ©ation d'un groupe unique")
        return [{"sites": sites, "date_min": None, "date_max": None}]
    
    log.info(f"ðŸ“… {len(dated_sites)} sites avec dates sur {len(sites)} sites totaux")
    
    # Trier par date_min
    dated_sites.sort(key=lambda x: x["date_min"] or datetime.min)
    
    # CrÃ©er des groupes de sites compatibles (intervles qui se chevauchent)
    groups = []
    ...
    for site in dated_sites:
        ...
        if not current_group["sites"]:
            ...
            # VÃ©rifier compatibilitÃ© avec le groupe actuel
            group_min = current_group["date_min"]
            group_max = current_group["date_max"]
            
            # VÃ©rifier si les intervalles se chevauchent
            intervals_overlap = (
            ...
            if intervals_overlap:
                # Ajouter au groupe existant et Ã©tendre l'intervalle
                current_group["sites"].append(site)
                ...
                # Sauvegarder le groupe actuel et en crÃ©er un nouveau
    ...
    log.info(f"ðŸ“… {len(groups)} groupes de dates identifiÃ©s")
    ...
    if sites_without_dates and groups:
        log.info(f"ðŸ“… {len(sites_without_dates)} sites sans dates ajoutÃ©s au premier groupe")
        groups[0]["sites"].extend(sites_without_dates)
    elif not groups and sites_without_dates:
        # Si aucun groupe avec dates, crÃ©er un groupe unique
...
def smart_merge_routes(routes, sites, time_m, dist_m, depot_idx, min_sites, max_sites, max_time):
    """
    Fusionne les tournÃ©es trop courtes avec leurs voisines gÃ©ographiques
    pour respecter le minimum de sites, sans dÃ©passer les contraintes.
    """
    merged = True
    # On travaille sur une copie des routes (liste de dicts)
    current_routes = [{"route": list(r["route"])} for r in routes]
    
    log.info(f"  ðŸ”„ Smart Merge : {len(current_routes)} tournÃ©es initiales (Min sites: {min_sites})")

    while merged:
        merged = False
        
        # Trouver les tournÃ©es trop courtes
        small_tour_indices = [i for i, r in enumerate(current_routes) if len(r["route"]) < min_sites]
        
        if not small_tour_indices:
            break
            
        # Tenter de fusionner chaque petite tournÃ©e
        for idx in small_tour_indices:
            if idx >= len(current_routes): continue # DÃ©jÃ  supprimÃ©e ?
            if len(current_routes[idx]["route"]) >= min_sites: continue # DÃ©jÃ  fusionnÃ©e dans ce tour de boucle
            
            route_a = current_routes[idx]["route"]
            if not route_a: continue # Route vide ?

            # Trouver la tournÃ©e la plus proche gÃ©ographiquement (simple heuristique : fin de A proche du dÃ©but de B)
            best_neighbor_idx = -1
            min_dist_to_b = float('inf')
            
            # Dernier site de la tournÃ©e A
            ...
            for j, route_dict_b in enumerate(current_routes):
                ...
                # C'est une heuristique rapide avant de vÃ©rifier le temps rÃ©el
            ...
            # Si un voisin est trouvÃ©, vÃ©rifier si la fusion est valide en temps
            if best_neighbor_idx != -1:
                ...
                # Temps de transition entre fin de A et dÃ©but de B
                ...
                if total_merged_time <= max_time:
                    # FUSION VALIDÃ‰E
                    # Nouvelle route : A -> B (ou B -> A, ici on concatÃ¨ne simplement)
                    # Note: L'ordre optimal serait A puis B (on finit A, on va au dÃ©but de B)
                    ...
                    log.info(f"  âœ… Fusion : TournÃ©e {idx+1} ({len(route_a)} sites) + TournÃ©e {best_neighbor_idx+1} ({len(route_b)} sites)")
                    merged = True
                    break # On sort de la boucle pour recommencer l'analyse

    # Reconvertir en format attendu par la suite (avec metadonnÃ©es vides pour les nouvelles routes)
    final_routes = []
    ...
    log.info(f"  ðŸ Smart Merge terminÃ© : {len(final_routes)} tournÃ©es finales.")
    return final_routes


def full_pipeline_enhanced(filepath, params, session_id, progress_id=""):
    depot_name   = params["depot_site"]
    max_hours    = float(params.get("max_tour_hours", 7.0))  # Nouveau paramÃ¨tre: durÃ©e max par tournÃ©e
    ...
    log.info(f"â±ï¸  Mode planification basÃ© sur le TEMPS (max {max_hours}h/tournÃ©e)")

    # 1. Lecture
    log.info("=== 1/7 Lecture Excel ===")
    _prog(5, "Lecture du fichierâ€¦")
    df = pd.read_excel(filepath, dtype=str, na_filter=False, keep_default_na=False)
    if df.empty:
        raise ValueError("Le fichier Excel est vide.")
    
    # Colonnes optionnelles avec valeurs par dÃ©faut
    ...
    if "Temps" in df.columns:
        # Priorité à la colonne métier 'Temps' (en heures) si elle est renseignée.
        hours_mask = df["Temps"].astype(str).str.strip().ne("")
        if hours_mask.any():
            df.loc[hours_mask, "temps_site"] = df.loc[hours_mask, "Temps"].apply(_heures_to_minutes)
            log.info("  â±ï¸  Colonne 'Temps' (heures) appliquée à %d lignes (prioritaire)", int(hours_mask.sum()))
    else:
        ...
        if len(ts_valid) >= 3 and ts_valid.max() <= 12 and ts_valid.median() <= 8:
            ...
            log.warning("  âš ï¸  'Temps' absent: 'temps_site' ressemble à des heures, conversion auto heures→minutes appliquée")

    empty_mask = df["temps_site"].astype(str).str.strip().eq("")
    if empty_mask.any():
        df.loc[empty_mask, "temps_site"] = "30"
        log.info("  â±ï¸  Valeur par défaut 30 min appliquée à %d lignes", int(empty_mask.sum()))

    log.info("  â±ï¸  Colonne 'temps_site' détectée - Temps de visite personnalisé activé")
    log.info("  ðŸ“… Colonnes dÃ©tectÃ©es: %s", list(df.columns))

    raw_rows = df.to_dict("records")
    log.info("  âœ… %d lignes lues", len(raw_rows))

    # 2. GÃ©ocodage
    log.info("=== 2/7 GÃ©ocodage avec validation ===")
    _prog(10, f"VÃ©rification des coordonnÃ©es ({len(raw_rows)} sites)â€¦")

    # DÃ©tection si donnÃ©es prÃ©-gÃ©ocodÃ©es
    is_pre_geocoded = any(
        pd.notna(r.get("_lat")) and pd.notna(r.get("_lon")) 
        for r in raw_rows
    )
    log.info(f"  ðŸ” DonnÃ©es prÃ©-gÃ©ocodÃ©es dÃ©tectÃ©es : {is_pre_geocoded}")

    if is_pre_geocoded:
        # CAS 1 : DonnÃ©es dÃ©jÃ  gÃ©ocodÃ©es (vient de l'Ã©tape validation)
        all_geocoded_sites = []
        for r in raw_rows:
            # Conversion string vers float pour Ã©viter les erreurs de logs/calculs
            try:
        ...
        log.info(f"  âœ… {len(all_geocoded_sites)} sites utilisÃ©s (coordonnÃ©es existantes)")
        
        # VÃ©rification prÃ©sence DÃ©pÃ´t et ajout manuel si manquant
        depot_exists = _find_depot_index(all_geocoded_sites, depot_name) is not None
        
        if not depot_exists:
            log.warning(f"  ðŸšš Le dÃ©pÃ´t '{depot_name}' n'est pas dans le fichier. Ajout manuel depuis les donnÃ©es brutes...")
            ...
            if raw_depot_idx is not None:
                ...
                try:
                    ...
                    if not math.isfinite(lat) or not math.isfinite(lon):
                        raise ValueError("CoordonnÃ©es non finies")
                    r["_lat"] = lat
                    r["_lon"] = lon
                    r["_date_min"] = ""
                    r["_date_max"] = ""
                    all_geocoded_sites.insert(0, r)
                    log.info(f"  âœ… DÃ©pÃ´t ajoutÃ© manuellement en position 0 (lat={lat:.5f}, lon={lon:.5f})")
                    added = True
                except (ValueError, TypeError):
                    pass
                if not added:
                    # Fallback : gÃ©ocoder l'adresse du dÃ©pÃ´t
                    depot_addr  = str(r.get("address", "")).strip()
                    depot_ville_str = str(r.get("ville", "")).strip()
                    query = ", ".join(filter(None, [depot_addr, depot_ville_str]))
                    if query:
                        log.warning(f"  ðŸ” GÃ©ocodage du dÃ©pÃ´t '{depot_name}': {query}")
                        ...
                        r["_date_max"] = ""
                        all_geocoded_sites.insert(0, r)
                        log.info(f"  âœ… DÃ©pÃ´t gÃ©ocodÃ© et ajoutÃ© en position 0 (lat={geo['lat']:.5f}, lon={geo['lon']:.5f})")
                    else:
                        log.error(f"  âŒ DÃ©pÃ´t '{depot_name}' sans coordonnÃ©es ni adresse valide")
        
        sites = all_geocoded_sites
        
    else:
        # CAS 2 : GÃ©ocodage API
        ...
        if geocode_results["manual_needed"] > 0:
            return {
                ...
                "stage": "geocoding",
                "needs_manual": True,
                "message": f"{geocode_results['manual_needed']} adresses nÃ©cessitent une correction manuelle",
                "geocode_results": geocode_results,
                "session_id": session_id
            }
        sites = geocode_results["success"]

    _prog(55, "CoordonnÃ©es chargÃ©es â€” analyse des datesâ€¦")

    # 2.5 Gestion des Dates (MÃ©tadonnÃ©es uniquement - Pas de contrainte VRP)
    # C'est ici que l'on met votre bloc de code
    log.info("=== 2.5/7 Analyse des dates (MÃ©tadonnÃ©es) ===")
    ...
    log.info(f"  âœ… MÃ©tadonnÃ©es de dates ajoutÃ©es Ã  {len(sites)} sites")

    # 3. DÃ©pÃ´t
    log.info("=== 3/7 DÃ©pÃ´t ===")
    depot_idx = _find_depot_index(sites, depot_name)
    if depot_idx is None:
        raise ValueError(f"DÃ©pÃ´t '{depot_name}' introuvable.")
    
    depot_site = sites[depot_idx]
    log.info("  DÃ©pÃ´t idx=%d  lat=%.5f lon=%.5f", depot_idx,
             depot_site["_lat"], depot_site["_lon"])

    # 4. Matrice
    log.info("=== 4/7 Matrice OSRM (%s) ===", profil)
    _prog(60, f"Calcul de la matrice de distances ({len(sites)} sites)â€¦")
    time_m, dist_m = build_matrices(sites, profil)

    BIG = 10_000_000
    n = len(sites)
    non_big = sum(1 for i in range(n) for j in range(n) if i != j and time_m[i][j] < BIG)
    log.info("  Paires renseignÃ©es : %d / %d", non_big, n*(n-1))

        # 5. VRP Global (BasÃ© sur le temps)
    log.info("=== 5/7 VRP OR-Tools (BasÃ© sur le temps) ===")
    _prog(70, "Optimisation des tournÃ©es (OR-Tools)â€¦")

    # Nouveau paramÃ¨tre: temps max par tournÃ©e en heures
    max_tour_hours = float(params.get("max_tour_hours", 7.0))
    log.info(f"â±ï¸  Contrainte: Max {max_tour_hours}h par tournÃ©e")

    if split_mode == "auto":
        # Mode auto : le VRP crée un seul grand parcours (TSP) sans contrainte
        # de temps, puis split_long_tours_by_time le découpe en tournées
        # de durée variable selon max_tour_hours + durée de visite.
        log.info("  🔄 Mode auto : résolution TSP (1 véhicule sans contrainte de temps)")
    ...
    if not raw_routes:
        raise RuntimeError("OR-Tools : aucune solution trouvÃ©e.")

    log.info(f"  ðŸ“ VRP Brut : {len(raw_routes)} tournÃ©es trouvÃ©es.")

    # DÃ©coupage des routes trop longues (basÃ© sur le temps incluant les visites)
    ...
    log.info(f"  âœ… {len(routes)} tournÃ©es finales.")
    ...
    if use_ai:
        ...
        _prog(85, "Analyse IA des tournÃ©esâ€¦")
        ai_result = analyze_routes_with_ai(routes, sites, time_m, dist_m, depot_idx)
        if ai_result["success"]:
            ai_analysis = ai_result
            log.info("  âœ… Analyse IA terminÃ©e")
        else:
            log.warning("  âš ï¸  Analyse IA Ã©chouÃ©e")
            ai_analysis = { "success": False, "message": "Erreur analyse" }

    # 7. RÃ©sultats
    log.info("=== 7/7 RÃ©sultats ===")
    _prog(93, "GÃ©nÃ©ration du fichier Excelâ€¦")
    ...
    def route_totals(route):
        ...
        """
        # Temps de trajet : DÃ©pÃ´t -> Route -> DÃ©pÃ´t
        seq = [depot_idx] + route + [depot_idx]
    ...
    for tour_num, route_data in enumerate(routes, 1):
        ...
        if ts > max_s * 0.9:
            issues.append({
                "tour": tour_num,
                "type": "long_tour",
                "message": f"TournÃ©e #{tour_num} proche de la durÃ©e maximale ({ts/3600:.1f}h / {max_s/3600:.1f}h)"
            })
        
        # Log dÃ©taillÃ© par tournÃ©e
        log.info(f"  ðŸ“ T{tour_num}: {len(route)} sites, {dm/1000:.1f}km, {ts/3600:.1f}h (trajet: {(ts-visit_time)/3600:.1f}h + visite: {visit_time/3600:.1f}h)")

        # Construction de la route pour la carte et le JSON
        route_coords = [[depot_site["_lat"], depot_site["_lon"]]]
        stops = []
        
        # IMPORTANT : On filtre le dÃ©pÃ´t pour ne pas l'afficher comme un site
        for si in route:
            s = sites[si]
            # Double vÃ©rification : si c'est le dÃ©pÃ´t, on passe
            if si != depot_idx:
                stops.append({
                    "site":    str(s.get("site",   "")),
        ...
        # On revient au dÃ©pÃ´t pour fermer la boucle visuelle
        route_coords.append([depot_site["_lat"], depot_site["_lon"]])

        # Calcul de la fenÃªtre de dates valide pour cette tournÃ©e
        # (intersection des fenÃªtres de tous les sites de la tournÃ©e)
        ...
        if tour_dates_min and tour_dates_max:
            ...
            if t_min <= t_max:
                ...
                tour_date_proposed = "âš ï¸ Conflit de dates"
                issues.append({
                    "tour": tour_num,
                    "type": "date_conflict",
                    "message": f"TournÃ©e #{tour_num} : les fenÃªtres de dates des sites sont incompatibles"
                })
        elif tour_dates_min:
        ...
        tour_observation = "TournÃ©e optimisÃ©e."
...
def api_upload_validate():
    """
    Ã‰tape 1 : Upload et validation/gÃ©ocodage des adresses
    Retourne les adresses validÃ©es et celles nÃ©cessitant une correction
    """
    try:
        if "file" not in request.files:
            return jsonify({"success": False, "error": "Aucun fichier uploadÃ©"}), 400
        ...
        try:
            set_progress(progress_id, 5, "Lecture du fichierâ€¦")
            df = pd.read_excel(fpath, dtype=str, na_filter=False, keep_default_na=False)
            if df.empty:
                raise ValueError("Le fichier Excel est vide.")

            # Appliquer le mappage de colonnes reÃ§u du client
            if column_mapping:
                df = apply_column_mapping(df, column_mapping)
                log.info("  ðŸ“Œ Mappage appliquÃ© : %s", column_mapping)

            # Colonnes obligatoires manquantes â†’ valeur vide
            for col in ["site", "address", "ville"]:
                if col not in df.columns:
                    df[col] = ""

            # Colonnes optionnelles : si absentes, crÃ©er vides
            for col in ["date_min", "date_max", "temps_site"]:
                if col not in df.columns:
                    df[col] = ""

            # Harmonisation du temps de visite :
            # - 'Temps' est en heures dans le fichier mÃ©tier
            ...
            if "Temps" in df.columns:
                ...
                if hours_mask.any():
                    ...
                    log.info("  â±ï¸  Upload: 'Temps' (heures) converti vers 'temps_site' (minutes) sur %d lignes", int(hours_mask.sum()))

            raw_rows = df.to_dict("records")
            set_progress(progress_id, 10, f"GÃ©ocodage de {len(raw_rows)} adressesâ€¦")

            def _geo_cb(pct, msg):
                set_progress(progress_id, pct, msg)

            geocode_results = geocode_with_validation(raw_rows, session_id, progress_callback=_geo_cb)
            set_progress(progress_id, 100, "GÃ©ocodage terminÃ© âœ“", done=True)
...
def api_manual_geocode():
    """
    Permet de corriger manuellement les adresses et de les gÃ©ocoder
    """
    try:
        data = request.json
        ...
        log.info(f"Correction manuelle demandÃ©e - Session: {session_id}, Corrections: {len(corrections)}")
        ...
        for corr in corrections:
            ...
            if idx is not None and (new_address or (lat and lon)):
                # Retrouver l'entrÃ©e dans failed par son index original
                failed_entry = next(
                ...
                if failed_entry:
                    # PrioritÃ© aux coordonnÃ©es directes
                    if lat and lon:
                        try:
                            failed_entry["_lat"] = float(lat)
                            failed_entry["_lon"] = float(lon)
                            failed_entry["_display_name"] = f"CoordonnÃ©es manuelles: {lat}, {lon}"
                            ...
                            log.info("âœ… CoordonnÃ©es manuelles appliquÃ©es pour index %d", idx)
                        except (TypeError, ValueError) as e:
                            log.error("âŒ Erreur coordonnÃ©es: %s", e)
                            continue
                    # Sinon, gÃ©ocodage avec nouvelle adresse
                    elif new_address:
                        ...
                        if geo_result:
                            ...
                            log.info("âœ… GÃ©ocodage rÃ©ussi pour index %d", idx)
                        else:
                            # Ceci ne devrait plus jamais arriver
                            log.warning("âš ï¸  GÃ©ocodage Ã©chouÃ© pour index %d (impossible)", idx)
                            continue
                    
                    # DÃ©placer vers success
                    ...
                    log.warning("âš ï¸  EntrÃ©e non trouvÃ©e pour index %d", idx)
        
        # Mettre Ã  jour le stockage
        temp_data_store[session_id] = results
        
        log.info("âœ… Correction terminÃ©e: %d adresses corrigÃ©es", corrected_count)
...
def api_optimize():
    """
    Ã‰tape 2 : Optimisation complÃ¨te avec les adresses validÃ©es
    """
    try:
        ...
        if not data or "session_id" not in data:
            # --- Fallback pour l'ancienne mÃ©thode avec upload de fichier ---
            if "file" not in request.files:
            ...
            # --- Nouvelle mÃ©thode avec session_id ---
            ...
            # RÃ©cupÃ©rer les rÃ©sultats de gÃ©ocodage
            if session_id not in temp_data_store:
                return jsonify({"success": False, "error": "Session expirÃ©e"}), 400
            ...
            # CrÃ©er un fichier temporaire avec les sites gÃ©ocodÃ©s
            sites = geocode_results["success"]

            # CrÃ©er un DataFrame temporaire
        ...
        set_progress(progress_id, 100, "Optimisation terminÃ©e âœ“", done=True)
...
def api_move_stop():
    """
    DÃ©place un stop d'une tournÃ©e Ã  une autre ou rÃ©ordonne dans la mÃªme tournÃ©e.
    """
    try:
        data = request.json
        session_id = data.get("session_id")
        ...
        if not tours or from_tour is None or from_idx is None or to_tour is None or to_idx is None:
            return jsonify({"success": False, "error": "ParamÃ¨tres manquants"}), 400

        src = next((t for t in tours if t["tour_num"] == from_tour), None)
        dst = next((t for t in tours if t["tour_num"] == to_tour), None)

        if not src or not dst:
            return jsonify({"success": False, "error": "TournÃ©e introuvable"}), 400
        ...
        # Ajuster l'index si mÃªme tournÃ©e et dÃ©placement vers l'avant
        insert_idx = to_idx
        ...
        return jsonify({
            "success": True,
            "tours": tours,
            "message": f"Stop dÃ©placÃ©: T{from_tour}[{from_idx}] â†’ T{to_tour}[{insert_idx}]"
        })
...
def api_recalculate_tour():
    """
    Recalcule les temps/distances d'une tournÃ©e modifiÃ©e manuellement via OSRM.
    """
    try:
        data = request.json
        session_id = data.get("session_id")
        ...
        if not depot_site:
            return jsonify({"success": False, "error": "DÃ©pÃ´t introuvable"}), 400
...
def api_auto_split():
    """
    Calcule automatiquement le split_size optimal (nb de sites par tournÃ©e)
    en fonction du temps de trajet rÃ©el (OSRM) + temps de visite des sites.

    Algorithme :
    1. RÃ©cupÃ¨re les sites gÃ©ocodÃ©s depuis la session
    2. Calcule la matrice OSRM (ou utilise une estimation heuristique si trop de sites)
    3. Simule des tournÃ©es type et dÃ©termine combien de sites tiennent dans max_tour_hours
    4. Retourne split_size + explication dÃ©taillÃ©e
    """
    try:
        data = request.json
        session_id   = data.get("session_id")
        ...
        if not client_sites:
            return jsonify({"success": False, "error": "Aucun site client trouvÃ©"}), 400

        n_sites = len(client_sites)
        max_s   = max_hours * 3600  # secondes

        # â”€â”€ Calcul du temps de visite moyen â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        visit_times = []
        ...
        # â”€â”€ Estimation du temps de trajet inter-sites â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        # On calcule la matrice OSRM sur un Ã©chantillon (max 20 sites)
        # pour avoir un temps de trajet moyen rÃ©aliste
        ...
        log.info("ðŸ”¢ Auto-split: calcul OSRM sur %d sites (Ã©chantillon)", len(sample_with_depot))

        try:
            time_m_sample, dist_m_sample = build_matrices(sample_with_depot, profil)
            depot_i = 0 if depot_site else None

            # Temps de trajet moyen entre deux sites consÃ©cutifs dans une tournÃ©e optimale
            ...
            # Trajet dÃ©pÃ´t â†’ premier site et dernier site â†’ dÃ©pÃ´t
            ...
            use_osrm = True
            log.info("âœ… OSRM OK â€” trajet moyen inter-sites: %.0fs (%.1f min)", avg_travel_s, avg_travel_s/60)

        except Exception as e:
            log.warning("âš ï¸  OSRM Ã©chec (%s), estimation heuristique", e)
            avg_travel_s       = 1800   # 30 min entre sites
            avg_depot_travel_s = 3600   # 1h aller-retour dÃ©pÃ´t
            use_osrm = False

        # â”€â”€ Simulation : combien de sites tiennent dans max_s ? â”€â”€
        # On simule une tournÃ©e "type" :
        # temps_total = depotâ†’s1 + (n-1)*avg_inter + snâ†’depot + sum(visites)
        # On cherche le plus grand n tel que temps_total â‰¤ max_s

        best_n = 1
        for n in range(1, n_sites + 1):
            # Temps de trajet estimÃ© pour n sites
            if n == 1:
                travel = avg_depot_travel_s  # aller-retour direct
            else:
                travel = avg_depot_travel_s + (n - 1) * avg_travel_s

            # Temps de visite : moyenne pondÃ©rÃ©e sur les n premiers sites
        ...
        # Garde-fous Auto:
        # - minimum 2 sites/tournÃ©e (si possible)
        # - maximum 8 sites/tournÃ©e
        if n_sites >= 2:
            best_n = max(2, min(best_n, 8, n_sites))
        else:
            best_n = max(1, min(best_n, n_sites))

        # â”€â”€ Calcul du nombre de tournÃ©es estimÃ© â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        estimated_tours = math.ceil(n_sites / best_n)

        # â”€â”€ DÃ©tail pÃ©dagogique â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        detail = (
            f"{n_sites} sites Â· visite moy. {avg_visit_s/60:.0f} min/site Â· "
            f"trajet moy. {avg_travel_s/60:.0f} min/inter Â· "
            f"aller-retour dÃ©pÃ´t ~{avg_depot_travel_s/60:.0f} min â†’ "
            f"{best_n} sites/tournÃ©e ({total_for_best/3600:.1f}h/{max_hours}h, marge {margin_s/60:.0f} min) Â· "
            f"~{estimated_tours} tournÃ©es estimÃ©es"
        )

        log.info("âœ… Auto-split calculÃ©: %d sites/tournÃ©e (%s)", best_n, "OSRM" if use_osrm else "heuristique")
...
def api_test_ollama():
    ...
    try:
        response = ask_ollama("Test. RÃ©ponds juste 'OK'.", timeout=60)
        if response and len(response) > 0:
            # VÃ©rifier que la rÃ©ponse n'est pas vide
            return jsonify({
                "success": True,
                "message": "Ollama connectÃ© et fonctionnel",
                "model": OLLAMA_MODEL,
                "response_preview": response[:100]  # AperÃ§u pour debug
            })
        else:
            return jsonify({
                "success": False,
                "message": "Ollama connectÃ© mais ne rÃ©pond pas",
                "response": response
            })
    except requests.exceptions.Timeout:
        return jsonify({
            "success": False,
            "message": "Timeout - Ollama trop lent"
        }), 200
    except requests.exceptions.ConnectionError:
        return jsonify({
            "success": False,
            "message": "Ollama non connectÃ© - VÃ©rifiez qu'il tourne"
        }), 200
    except Exception as e:
        return jsonify({
            "success": False,
...
if __name__ == "__main__":
    print("=" * 70)
    print("  ðŸšš  VRP Optimizer Pro v4")
